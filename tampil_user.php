 ...
 <table align="center"  background="image/t.gif" width="890" class="kecil">
  ...
  <tr>
    <td width="101"><h3>Nama User</h3></td>
     <td width="106"><h3>NO Telp</h3></td>
    <td width="106"><h3>Alamat Email</h3></td>
    <td width="145"><h3>Nama Perusahan</h3></td>
    <td width="146"><h3>Tanggal Bergabung</h3></td>
    <td colspan="2" align="center"><h3>AKSI</h3></td>
    </tr>
  <?php
  include "koneksi.php";
  $sql_news="SELECT id,username,tlp,email,company,tgl FROM users ORDER BY id";
  ...
  while($data=mysql_fetch_array($qry_news)){
	...
	<td><? echo $data[email]; ?></td>
      ...
      <td> <? echo $data[tgl]; ?></td>
      <td width="60" align="center"><a href="hapus.php?id=<? echo $data[id] ?>">hapus</a>
      <td width="54" align="center"><a href="ubah.php?id=<? echo 
$data[id] ?>">Ubah</a></td>
 </tr>
 <?php
  }
